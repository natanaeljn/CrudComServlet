...
public class DAOUsuarioRepository {
	...
	public beanDtoGraficoSalario montarGraficoMediaSalarioData(Long userLogado, String dataInicial, String dataFinal)
			throws Exception {
		String sql = " select avg(renda) as media_salarial, perfil from model_login where usuarioid = ? and datanascimento >= ? and datanascimento <= ? group by perfil ";
	...
	public ModelLogin gravarUsuario(ModelLogin obj, Long getUserLogado) throws Exception {
		...
		if (obj.isNovo()) {
			...
			String sql = "UPDATE model_login SET login=?, senha=?,  nome=?, email=? , perfil=? , sexo=? , cep=? , logradouro=? , bairro=?, localidade=? , uf=? , numero=? , datanascimento=? , renda=?  WHERE id = "
					+ obj.getId();
	...
	public List<ModelLogin> consultaUsuarioListPaginada(String nome, Long getUserLogado, Integer offset)
			throws Exception {
	...
	public List<ModelLogin> consultaUsuarioListOffset(String nome, Long getUserLogado, int offset) throws Exception {
		List<ModelLogin> retorno = new ArrayList<ModelLogin>();
		String sql = "select * from model_login where upper(nome) like upper(?) and  useradmin  is false and usuarioid = ?  offset "
				+ offset + "  limit 5";
		PreparedStatement statement = connection.prepareStatement(sql);
		statement.setString(1, "%" + nome + "%");
		statement.setLong(2, getUserLogado);
	...
	public ModelLogin consultarUsuario(String login, Long getUserLogado) throws Exception {
		ModelLogin modelLogin = new ModelLogin();
		String sql = "select * from model_login where upper(login) = upper( '" + login
				+ "' ) and  useradmin  is false and usuarioid= " + getUserLogado;
		PreparedStatement statement = connection.prepareStatement(sql);
		ResultSet resultado = statement.executeQuery();
		while (resultado.next())/* se tiver um resultado */ {
			modelLogin.setId(resultado.getLong("id"));
	...
	public List<ModelLogin> consultaUsuarioRelatorioDatas(Long userLogado, String dataInicial, String dataFinal)
			throws Exception {
		List<ModelLogin> retorno = new ArrayList<ModelLogin>();
		String sql = "select * from model_login where useradmin  is false and usuarioid = " + userLogado
